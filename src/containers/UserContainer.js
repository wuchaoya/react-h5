...
class User extends Component {
  ...
  render () {
    ...
    return data ? <Container background='#fff' marginBottom={0.56}>
      ...
      <UserEquity>
        ...
        {data.map((item, index) => {
          return <EquityItem index={index} key={index}>
  ...
  componentDidMount () {
    ...
    document.title = '我的';
    this.getData();
    window.addEventListener('message', this.setOpenView);
  }
  setOpenView (event) {
    console.log(event);
    if (event.origin === 'http://pcloud.haimawan.com') {
      this.getMyService();
      this.setState({
        openView: false
      });
  ...
  componentWillUnmount () {
    window.removeEventListener('message', this.setOpenView);
    console.log('组件卸载前');
  }
};
